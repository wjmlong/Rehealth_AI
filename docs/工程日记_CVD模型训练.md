...
| # | 问题 | 严重度 | 说明 |
|---|------|--------|------|
| 1 | `risk_scorer.py:75` 硬编码 `model_auc: 0.847` | 🟡 | 应从模型元数据读 |
| 2 | V2 模型 AUC 偏低 (0.767) | 🟡 | 归因效果方向可能反直觉，需 16 特征重训 |
| 3 | `/prescription` 不传 risk_result 时把 memory_snapshot 当特征 | 🟡 | `inference.py:45` 逻辑错误 |
| 4 | `/attribution/group` 用 `hash() % N` 编码特征 | 🟡 | 不稳定，应用明确映射 |
| 5 | PSM 匹配平衡性 4/7 SMD>0.1 | 🟢 | 有诊断报告，数据量上去后会改善 |
